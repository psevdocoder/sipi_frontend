...
export default {
  ...
  setup() {
    ...
    const submit = async () => {
      ...
      if (response.status == 400 || response.status == 401) { // если код ответа 400, данные авторизации неверны
        const errorResponse = await response.json();
        data.errorMessage = errorResponse.detail;
      } else {
        const responseData = await response.json();
        document.cookie = `jwt=${responseData.access}`; // сохраняем токен JWT в куки
        await router.push('/')
      }
    }
